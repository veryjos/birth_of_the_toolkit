  ...
  internal class Archive {
    ...
    public Archive(Stream stream) {
      ...
      if (header[0] == 'Y') {
        ...
        Yaz0.Decompress(stream,
          (MemoryStream)
          archiveFileStream); // need to do something better then this whacky shit, need a just in time reader
        stream.Close();
        stream.Dispose();
        stream = null;
      }
      ...
      for (int i = 0; i < sfatEntries.Length; i++) {
        ...
        fileDictionary.Add(name,
          new ArchiveEntry(name, sfatEntries[i].DataOffsetStart + archiveHeader.DataOffset, size,
            sfatEntries[i].FileType));
      }
    }
