...
fileNamePattern = heightmapDir + "/5%(lodLevel)d0000%(index)s.mate"
...
outPixels = [0] * outWidth * outHeight * 4 
        ...
        try:
            ...
            for py in range(0, tileHeight):
                for px in range(0, tileWidth):
                    ...
                    outPixels[((sy + py) * outWidth + (sx + px)) * 4] = pr
                    outPixels[((sy + py) * outWidth + (sx + px)) * 4 + 1] = 0
                    outPixels[((sy + py) * outWidth + (sx + px)) * 4 + 2] = pb
                    outPixels[((sy + py) * outWidth + (sx + px)) * 4 + 3] = pa
